...
class OpStudentCourse(models.Model):
    ...
    subject_grades_ids = fields.One2many('op.subject.grade.student', 'student_id', string="Subject Grades")
    ...
    def create(self, vals):
        # Call the super to create the student course record
        record = super(OpStudentCourse, self).create(vals)
        # Automatically create student grades when subjects are assigned
        if 'subject_ids' in vals:
            for subject in record.subject_ids:
                self.env['op.subject.grade.student'].create({
                    'student_id': record.student_id.id,
                    'subject_id': subject.id,
                    'course_id': record.course_id.id,
                    'academic_year_id': record.academic_years_id.id,
                    'academic_term_id': record.academic_term_id.id,
                    'grade': 0.0,  # Initialize grade to 0 or some default value
                })
        return record

    def write(self, vals):
        # Call the super to write the student course record
        result = super(OpStudentCourse, self).write(vals)
        # Automatically create student grades when subjects are assigned
        if 'subject_ids' in vals:
            for record in self:
                for subject in record.subject_ids:
                    # Check if the student grade already exists
                    existing_grade = self.env['op.subject.grade.student'].search([
                        ('student_id', '=', record.student_id.id),
                        ('subject_id', '=', subject.id),
                        ('course_id', '=', record.course_id.id),
                        ('academic_year_id', '=', record.academic_years_id.id)
